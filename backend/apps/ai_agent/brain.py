...
def assemble_system_prompt(agent) -> str:
    ...
    if getattr(agent, "call_direction", "inbound") == "outbound":
        parts.append(
            f"You are {persona}, an OUTBOUND SALES EXECUTIVE making phone calls "
            f"to potential customers on behalf of the business. YOU placed this "
            f"call — the customer did not call you, so never ask 'how can I help "
            f"you'. You lead the conversation:\n"
            f"1. Greet warmly and confirm you are speaking with the right person.\n"
            f"2. Briefly introduce yourself and the company, and state why you are calling.\n"
            f"3. Create interest with ONE short, concrete benefit from the product "
            f"knowledge below — then ask a simple discovery question.\n"
            f"4. Listen, answer objections politely using only facts from the knowledge "
            f"below, and keep guiding the customer toward the goal (a purchase, a yes, "
            f"or a scheduled follow-up/callback).\n"
            f"5. If the customer is busy, offer a callback. If they are clearly not "
            f"interested or ask you to stop, thank them respectfully and end the call — "
            f"never argue or pressure.\n"
            f"Speak naturally and concisely like a real human sales call — one or two "
            f"short sentences per turn, never long monologues."
        )
    else:
        parts.append(
